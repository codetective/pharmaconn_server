              ...
              verify
                ...
                .then((newVerify) => {
                  ...
                  const mailOptions = {
                    ...
                    subject: "Welcome to Pharmaconn. Please verify your email.", // Subject
                    html: `<!DOCTYPE html>
<html>
<head>
                  ...
                  transport
                    ...
                    .catch((err) => {
                      res.send({
                        message: err,
                        error:
                          "email message sending failed, login and request a verification mail",
                      });
                    });
                })
                .catch((err) => {
  ...
  Verify.findOne({
    ...
    .then((verify) => {
      if (verify) {
        ...
        res.status(404).json({
          error:
            "Verification token not found in database, account may already be verified or does not exist",
        });
      }
    })
    .catch((err) => {
...
router.post("/verifymailrequest", (req, res) => {
    ...
    .then((user) => {
      Verify.findOne(
        ...
        (err, doc) => {
          ...
          if (doc) {
            console.log(doc);
            const link = `${process.env.CLIENT_URL}/verifyaccount?id=${doc.verificationCode}`;
            const mailOptions = {
              from: `Pharmaconnn ${process.env.MAIL_BOX}`, // sender
              to: req.body.email, // receiver
              subject: "Welcome to Pharmaconn. Please verify your email.", // Subject
...
<td bgcolor="#ffffff" align="left" style="padding: 0px 30px 20px 30px; color: #666666; font-family: 'Lato', Helvetica, Arial, sans-serif; font-size: 18px; font-weight: 400; line-height: 25px;">
    <p style="margin: 0;">If you have any questions, just reply to this email—we're always happy to help out.</p>
            ...
            transport
              ...
              .catch((err) => {
                ...
                res.status(500).json({
                  message: err,
                  error: "email message sending failed, retry later",
                });
              });
          }
        }
    ...
    .catch((err) => {
      console.log(err);
      res.status(400).json({
        error: "this email has not been registered in our database",
        notRegistered: true,
      });
    });
});
...
router.post("/login", (req, res) => {
  ...
  User.findOne({
    ...
    if (!user.isActive) {
      ...
      return res.status(400).json({
        error: "please verify your account. Check your email for instructions.",
      });
    }
...
router.post("/profile/bio/", auth, (req, res) => {
  console.log(res.locals.user);
  User.findOneAndUpdate({ _id: res.locals.user.id }, { bio: req.body.bio })
    .then((doc) => {
      console.log(doc);
      res.status(200).json({
        success: "bio updated",
...
router.post("/profile/updatedp", auth, (req, res) => {
  console.log(res.locals.user);
  if (req.files) {
    User.findOne({ _id: res.locals.user.id }).then((user) => {
      if (user.profileurl === false && user.dpId === false) {
        const data = {
          image: req.files.dp,
        };
        cloudinary.uploader.upload(
          ...
          (err, results) => {
            ...
            if (results) {
              ...
              const dp_id = results.public_id;
              User.findByIdAndUpdate(
                {
                  _id: res.locals.user.id,
                },
                { profileurl: image, dpId: dp_id },
                ...
                .then((doc) => {
                  ...
                  jwt.sign(
                    ...
                    (err, token) => {
                      res.status(200).json({
                        success: "image set successfully",
                        user: payload,
                        token: "Bearer " + token,
                      });
                    }
        ...
        console.log(user + "with old dp" + user.name);
        const data = {
          image: req.files.dp,
        };
        cloudinary.uploader.destroy(user.dpId, (err, results) => {
          if (err) {
            console.log(err);
            console.log("in uploading again");
            res.status(400).json({
