...
router.post(
  ...
  (req, res) => {
    ...
    newQuestion.save().then((question) => {
      ...
      newUpvote.save().then((upvote) => {
        Question.updateOne({ _id: question._id }, { upvotes: upvote._id }).then(
          () => {
            const newDownvote = new Downvote({
              questionid: question._id,
            });

            newDownvote.save().then((downvote) => {
              Question.updateOne(
                { _id: question._id },
                { downvotes: downvote._id }
              )
                .then(() => {
                  res.json(question);
                })
    ...
    socket.on("answerUpVoteClicked", (data) => {
      if (data.userid) {
        Question.aggregate([
          ...
          .then((answer) => {
            if (
              answer[0].answer[0].upvotes.filter(
                (vote) => vote.toString() === data.userid
              ).length === 0
            ) {
              if (
                answer[0].answer[0].downvotes.filter(
                  (vote) => vote.toString() === data.userid
                ).length > 0
              ) {
                Question.updateOne(
                  ...
                  .then(() => {
                    ...
                    Question.aggregate([
                      ...
                      .then((updatedAnswers) =>
                        io.emit("answerUpVoteUpdated", { updatedAnswers })
                      )
                      .catch((err) => console.log(err));
                  })
                  .catch((err) => console.log(err));
                ...
                Question.updateOne(
                  ...
                  .then(() => {
                    ...
                    Question.aggregate([
                      ...
                      .then((updatedAnswers) =>
                        io.emit("answerDownVoteUpdated", { updatedAnswers })
                      )
                      .catch((err) => console.log(err));
                  })
                  .catch((err) => console.log(err));
    ...
    socket.on("downvote", (downvote) => {
      if (downvote.userid) {
        Downvote.findOne({ questionid: downvote.questionid })
          .then((oneVote) => {
            if (
              oneVote.votes.filter(
                (vote) => vote.user.toString() === downvote.userid
              ).length === 1
            ) {
              Question.updateOne(
                { _id: downvote.questionid },
                {
                  $inc: { voteCount: 1 },
                }
              )
                    ...
                    .then(() => {
                      ...
                      Downvote.findOne({ questionid: downvote.questionid })
                        ...
                        .then((downvotedquestion) =>
                          io.emit("downvoteupdate", downvotedquestion)
                        )
                        .catch((err) => console.log(err));
                    })
                    .catch((err) => console.log(err));
      ...
      if (upvote.userid) {
        Upvote.findOne({ questionid: upvote.questionid })
          .then((oneVote) => {
            if (
              oneVote.votes.filter(
                (vote) => vote.user.toString() === upvote.userid
              ).length === 1
            ) {
              Question.updateOne(
